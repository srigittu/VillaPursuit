		...
		<table>
			<tr>
				...
				<td>
					...
					<div class="title" style="margin-top: 10px; padding: 2px;">
						...
						<span><c:out value="${viewAdvertisement.getHouseType()}" /></span>
					</div>
					<div class="title" style="padding: 2px;">
						<c:out value="${'Rent Type :'}" />
						<span><c:out value="${viewAdvertisement.getRentType()}" /></span>
					</div> <c:set value="${viewAdvertisement.getFacility()}" var="facility" />
					<div class="title" style="padding: 2px;">
				...
				<td style="padding: 10px;">
					<div class="title" style="margin-top: 10px; padding: 2px;">
						...
						<span><c:out value="${viewAdvertisement.getAvailability()}" /></span>
					</div>
				</td>
				<td>
					<div class="title" style="margin-top: 10px; padding: 2px;">
						...
						<br> <span><c:out
								value="${viewAdvertisement.getDate()}" /></span>
					</div>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan="3"><c:set
					...
					<div class="title" style="padding: 2px;">
						<span><c:out value="${address.getAddress()}" />
							<c:out value="${address.getArea()}" />,<br> <c:out
								value="${address.getCity()}" />, <c:out
								value="${address.getState()}" />, <c:out
								value="${address.getPincode()}" />.</span>
					</div></td>
				<td colspan="2"><c:set value="${viewAdvertisement.getUser()}"
						var="seller" />
					<div class="title">
					...
					<div class="title">
						Mobile: <span><c:out value="${seller.getPhoneNumber()}" /></span>
					</div>
					<div class="title">
			...
			<c:if
				test="${viewAdvertisement.getAdvertisementReviews() == null}">
